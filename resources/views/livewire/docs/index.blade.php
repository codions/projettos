...
<div class="sm:flex sm:items-center sm:justify-between">
    <div>
        <div class="flex items-center gap-x-3">
            <h2 class="text-lg font-bold text-gray-800 dark:text-white">{{ __('Explore Docs') }}</h2>
            <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">{{ $docs->total() }}</span>
        </div>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">{{ __('Documentation for our comprehensive projects.') }}</p>
    </div>

    <div class="mt-5 flex lg:mt-0 lg:ml-4">
        @if(auth()->check() && auth()->user()->hasAdminAccess())
        <a href="/admin/docs/create" class="ml-2 px-5 py-2 text-sm text-white bg-blue-500 rounded-lg sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600">
            <span>{{ __('Create') }}</span>
        </a>
        @endif
    </div>
...
<div class="mt-6 md:flex md:items-center md:justify-between">
    ...
    <div class="flex items-center space-x-4">
        <div class="relative flex items-center mt-4 md:mt-0">
            ...
            <input wire:model="search" type="search" placeholder="{{ __('Search all available docs') }}" class="block w-full py-1.5 pr-5 text-gray-700 bg-white border border-gray-200 rounded-lg md:w-80 placeholder-gray-400/70 pl-11 rtl:pr-11 rtl:pl-5 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40">
        </div>
...
<div class="flex text-sm text-gray-600 my-2">
    @if(!empty($search))
    <p class="mr-1 dark:text-gray-300">{!! __(':number prompts found for the search <strong>":search"</strong>.', ['number' => $docs->total(), 'search' => $search]) !!}</p>
...
<div class="grid 2xl:grid-cols-3 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-5">
    @foreach ($docs as $doc)
        <livewire:docs.card :doc="$doc" wire:key="doc-{{ $doc->id }}" />
    @endforeach
</div>
