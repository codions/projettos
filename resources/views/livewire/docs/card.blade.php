<div class="max-w-sm p-6 border rounded-lg shadow bg-white hover:bg-gray-50 border-gray-200 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
    <x-dynamic-component :component="$doc->project->icon ?? 'heroicon-o-hashtag'"
        class="w-7 h-7 text-gray-500 dark:text-gray-400 mb-3"/>

    <div class="flex flex-row">
        <div class="flex items-center w-full">
        <a href="{{ $doc->public_url }}">
            <h5 class="mb-2 text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ $doc->title }}</h5>
        </a>
        </div>
    ...
    <p class="mb-3 font-normal text-gray-500 dark:text-gray-400">{{ str_limit($doc->description, 150) }}</p>
    @endif
</div>
